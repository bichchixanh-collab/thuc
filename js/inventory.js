...
const Inventory = {
  ...
  useItem(itemId) {
    ...
    if (effect.hp)        { Player.heal(effect.hp, 0);       UI.addLog(`❤️ Sử dụng ${itemData.name}, +${effect.hp} HP`,           'heal');  }
    if (effect.mp)        { Player.heal(0, effect.mp);       UI.addLog(`💎 Sử dụng ${itemData.name}, +${effect.mp} MP`,           'heal');  }
    if (effect.exp)       { Player.gainExp(effect.exp);      UI.addLog(`⭐ Sử dụng ${itemData.name}, +${effect.exp} EXP`,          'exp');   }
    if (effect.realmExp)  { Player.gainRealmExp(effect.realmExp); UI.addLog(`✨ Sử dụng ${itemData.name}, +${effect.realmExp} Tu Vi`, 'realm'); }
  ...
  // Draw item icon on 16×16 canvas context
  drawItemIcon(ctx, itemId) {
    ...
    const icons = {
      ...
      wolfFang: () => {
        ctx.fillStyle = '#fff';
        ctx.beginPath(); ctx.moveTo(8, 2); ctx.lineTo(4, 14); ctx.lineTo(12, 14); ctx.closePath(); ctx.fill();
...
// ===== CHANGES: Xóa console.log debug; rút gọn else-nhánh trong render() thành ternary; gom if/else useItem thành một dòng mỗi effect; xóa comment thừa =====
