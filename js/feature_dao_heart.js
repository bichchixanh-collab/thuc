      ...
      case 'maBurst':
        if (typeof Enemies !== 'undefined' && Enemies.list) {
          Enemies.list.forEach(e => {
            ...
            if (dist <= 200) {
              const dmg = (Player.stats.atk || 10) * 3.0;
              if (typeof Enemies.damage === 'function') Enemies.damage(e, dmg);
            }
          });
        }
        break;
  ...
  tickDecay(dt) {
    ...
    if (this.state._decayTimer >= DAO_HEART_CONFIG.decayInterval) {
      ...
      if (this.state.score > 0) {
        this.addScore(-DAO_HEART_CONFIG.decayRate);
...
const TiemKiemPhoSystem = {
  ...
  _checkRealm(ch) {
    ...
    const stage = GameState.realmStage || 0;
    return realm >= ch.target && stage >= ch.targetStage;
  },
  ...
  tryUnlockPage(pageNum) {
    ...
    if (!this.checkChallenge(page)) return false;
  ...
  addProgress(key, amount) {
    if (key in this.state.progress) {
      this.state.progress[key] += amount;
    }
...
(function injectDaoStyles() {
  ...
  style.textContent = `
    /* ═══════════════════════════════════════════════════════
       DAO HEART — PANEL GỌN GÀO, KHÔNG CHE NÚT KHÁC
       Dùng position:fixed góc trái-dưới, có thể thu gọn
       ═══════════════════════════════════════════════════════ */
    ...
    #dao-root {
      ...
      bottom: 8px;
      left: 8px;
      z-index: 1500;          /* dưới modal (thường 2000+) nhưng trên game UI */
      display: flex;
      flex-direction: column;
      gap: 4px;
      pointer-events: none;   /* pass-through mặc định */
    ...
    .dao-skill-btn:hover:not(:disabled) {
      background: rgba(255,255,255,0.14);
    }

    .dao-skill-btn:disabled {
      opacity: 0.45;
      cursor: not-allowed;
    }

    .dao-skill-btn .dao-sk-icon {
      font-size: 14px;
      flex-shrink: 0;
    }

    .dao-skill-btn .dao-sk-info {
      display: flex;
      flex-direction: column;
      flex: 1;
      min-width: 0;
    ...
    .dao-skill-btn .dao-sk-cd {
      font-size: 9px;
      color: #f90;
      margin-top: 1px;
    }

    .dao-skill-btn .dao-sk-mp {
      font-size: 9px;
      color: #7cf;
      flex-shrink: 0;
    }
...
// ---------------------------------------------------------------------------
// DaoHeartHUD — thanh điểm + toggle panel
// ---------------------------------------------------------------------------
const DaoHeartHUD = {
  el: null,
  _collapsed: false,
  ...
  update() {
    ...
    // Score display
    const sign = score > 0 ? '+' : '';
    scoreEl.textContent = sign + score.toFixed(1);

    // Bar: center = 0, left = Ma(-100), right = Dao(+100)
    // Biểu diễn: width tính từ center ra theo chiều tương ứng
    const pct = Math.abs(score) / 100 * 50; // max 50%
    if (score >= 0) {
      // Đạo — bar từ center sang phải
      barEl.style.marginLeft = '50%';
      barEl.style.width = pct + '%';
      barEl.style.background = branch === 'dao' ? '#42a5f5' : (branch === 'honNguyen' ? '#e040fb' : '#aaa');
    } else {
      // Ma — bar từ center sang trái
...
// ---------------------------------------------------------------------------
// DaoSkillPanel — hiển thị 3 skill của branch hiện tại
// ---------------------------------------------------------------------------
const DaoSkillPanel = {
  el: null,
  ...
  // Gọi mỗi giây để refresh CD
  startCDTimer() {
    setInterval(() => {
      const branch = DaoHeartSystem.state.activeBranch;
      if (branch) DaoSkillPanel.refresh();
    }, 1000);
  }
};

// ---------------------------------------------------------------------------
// TiemKiemPhoUI — nút mở + modal
// ---------------------------------------------------------------------------
const TiemKiemPhoUI = {
  init() {
    this._createOpenBtn();
  ...
  _createModal() {
    ...
    document.getElementById('tkp-modal-close').addEventListener('click', () => this.closeModal());
    document.getElementById('tkp-kiem-than-btn').addEventListener('click', () => {
  ...
  refreshModal() {
    ...
    if (btn) {
      btn.disabled = !allUnlocked || onCD || TiemKiemPhoSystem.state.kiemThanActive;
    }
    if (statusEl) {
      if (!allUnlocked) {
        ...
        const hLeft = Math.ceil(cdLeft / 3600000);
        statusEl.textContent = `⏳ Hồi chiêu: ${hLeft}h`;
      } else {
...
(function patchGame() {
  // ── Game.init ──────────────────────────────────────────────────────────
  const _origInit = typeof Game !== 'undefined' ? Game.init?.bind(Game) : null;
  ...
  // ── Game.update ────────────────────────────────────────────────────────
  const _origUpdate = typeof Game !== 'undefined' ? Game.update?.bind(Game) : null;
  if (typeof Game !== 'undefined') {
  ...
  // ── Game.save ──────────────────────────────────────────────────────────
  const _origSave = typeof Game !== 'undefined' ? Game.save?.bind(Game) : null;
  if (typeof Game !== 'undefined') {
  ...
  // ── Game.load ──────────────────────────────────────────────────────────
  const _origLoad = typeof Game !== 'undefined' ? Game.load?.bind(Game) : null;
  if (typeof Game !== 'undefined') {
  ...
  // ── Player.recalculateStats ────────────────────────────────────────────
  const _origRecalc = typeof Player !== 'undefined' ? Player.recalculateStats?.bind(Player) : null;
  if (typeof Player !== 'undefined') {
    Player.recalculateStats = function(...args) {
      if (_origRecalc) _origRecalc(...args);
      // Kiếm Thần Hóa buff
      if (TiemKiemPhoSystem.state.kiemThanActive) {
  ...
  // ── Player.takeDamage ──────────────────────────────────────────────────
  const _origTakeDamage = typeof Player !== 'undefined' ? Player.takeDamage?.bind(Player) : null;
  if (typeof Player !== 'undefined') {
    Player.takeDamage = function(amount, ...args) {
      let dmg = amount;

      // Hỗn Nguyên: absorb → heal
      if (DaoHeartSystem.state._honBalanceActive) {
        const heal = dmg * 0.5;
        Player.hp = Math.min(Player.stats?.maxHp || Player.hp, Player.hp + heal);
      }
      ...
      // Kiếm Thần Hóa: immune = false, nhưng có thể mở rộng sau
      ...
      // Đạo: hứng đòn, không phản đòn → +score
      ...
      // Tiệm Kiếm Phổ: reset killNoHit streak
      TiemKiemPhoSystem.state.progress.killNoHitStreak = 0;
    };
  }

  // ── Player.useSkill ────────────────────────────────────────────────────
  const _origUseSkill = typeof Player !== 'undefined' ? Player.useSkill?.bind(Player) : null;
  if (typeof Player !== 'undefined') {
    Player.useSkill = function(skillId, ...args) {
      // Đạo Aura: giảm CD + miễn MP
  ...
  // ── Enemies.damage ──────────────────────────────────────────────────────
  const _origEnemyDmg = typeof Enemies !== 'undefined' ? Enemies.damage?.bind(Enemies) : null;
  if (typeof Enemies !== 'undefined') {
    Enemies.damage = function(enemy, amount, ...args) {
      let dmg = amount;
      // Ma Domain: player +30% dmg
      if (DaoHeartSystem.state._maDomainActive) dmg *= 1.30;
      // Hỗn Nguyên Transform: +100% dmg
      if (DaoHeartSystem.state._honTransformActive) dmg *= 2.0;
      // Ma Lifesteal
      if (DaoHeartSystem.state._maLifestealActive && typeof Player !== 'undefined') {
        const steal = dmg * 0.15;
        Player.hp = Math.min(Player.stats?.maxHp || Player.hp, Player.hp + steal);
      }
  ...
  // ── Enemies.kill ───────────────────────────────────────────────────────
  const _origEnemyKill = typeof Enemies !== 'undefined' ? Enemies.kill?.bind(Enemies) : null;
  if (typeof Enemies !== 'undefined') {
  ...
  // ── NPC.interact ───────────────────────────────────────────────────────
  const _origNPCInteract = typeof NPC !== 'undefined' ? NPC.interact?.bind(NPC) : null;
  if (typeof NPC !== 'undefined') {
    NPC.interact = function(npc, ...args) {
      ...
      if (npc?.type === 'event_help') {
        ...
        TiemKiemPhoSystem.addProgress('bossEventClears', 0); // only for boss
      }
...
// ===========================================================================
// SECTION 6 — AUTO INIT (nếu Game.init đã chạy trước khi script load)
// ===========================================================================

(function autoInit() {
  // Nếu DOM đã sẵn sàng, init HUD ngay
  function tryInit() {
    DaoHeartSystem.load();
    TiemKiemPhoSystem.load();
    DaoHeartHUD.init();
  ...
  if (document.readyState === 'loading') {
    ...
    // DOM đã ready — gọi sau 1 tick để chờ game modules khác
    setTimeout(tryInit, 50);
  }
})();

// ===========================================================================
// SECTION 7 — PUBLIC API (dùng từ console hoặc module khác)
// ===========================================================================

window.DaoHeartAPI = {
  // Thêm điểm Đạo/Ma thủ công (debug / event trigger)
  addScore: (n) => DaoHeartSystem.addScore(n),

  // Trigger events theo tên
  triggerEvent: (eventName) => {
    const val = DAO_HEART_CONFIG.events[eventName];
    if (val !== undefined) {
      DaoHeartSystem.addScore(val);
      console.log('[DaoHeart] event:', eventName, val);
    } else {
      console.warn('[DaoHeart] unknown event:', eventName);
    }
  },
