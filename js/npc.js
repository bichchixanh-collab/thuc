...
const NPC = {
  ...
  buildShopOptions(container) {
    ...
    itemsTab.style.cssText = `flex:1;padding:10px;border-radius:8px;cursor:pointer;font-family:inherit;font-size:11px;font-weight:bold;border:2px solid ${this.currentShopTab==='items'?'#f0c040':'#444'};background:${this.currentShopTab==='items'?'rgba(240,192,64,0.2)':'rgba(255,255,255,0.05)'};color:${this.currentShopTab==='items'?'#f0c040':'#888'};`;
    ...
    petsTab.style.cssText = `flex:1;padding:10px;border-radius:8px;cursor:pointer;font-family:inherit;font-size:11px;font-weight:bold;border:2px solid ${this.currentShopTab==='pets'?'#f0c040':'#444'};background:${this.currentShopTab==='pets'?'rgba(240,192,64,0.2)':'rgba(255,255,255,0.05)'};color:${this.currentShopTab==='pets'?'#f0c040':'#888'};`;
  ...
  buildPetsShop(container) {
    ...
    if (Player.activePet) {
      ...
      currentInfo.style.cssText = 'background:rgba(76,175,80,0.15);border:2px solid #4caf50;border-radius:8px;padding:10px;margin-bottom:12px;font-size:11px;color:#4caf50;display:flex;align-items:center;gap:8px;';
    ...
    for (const petId in PETS) {
      ...
      let bonusText = '';
      let bonusIcon = '';
      if (pet.bonus.atk) { bonusText = `+${pet.bonus.atk} ATK`; bonusIcon = '⚔️'; }
      else if (pet.bonus.def) { bonusText = `+${pet.bonus.def} DEF`; bonusIcon = '🛡️'; }
      else if (pet.bonus.speed) { bonusText = '+10% Speed'; bonusIcon = '💨'; }
      if (owned) {
        if (isActive) {
          option.style.borderColor = '#4caf50';
          option.style.background = 'rgba(76,175,80,0.15)';
          option.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center"><span>🐾 ${pet.name}</span><span style="color:#4caf50;font-size:10px">✓ Đang dùng</span></div><div style="font-size:9px;color:#8f8">${bonusIcon} ${bonusText} | +10% EXP</div>`;
          option.addEventListener('click', () => {
          ...
          option.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center"><span>🐾 ${pet.name}</span><span style="color:#2196f3;font-size:10px">📤 Triệu hồi</span></div><div style="font-size:9px;color:#888">${bonusIcon} ${bonusText} | +10% EXP</div>`;
          const pId = petId;
          option.addEventListener('click', () => {
        ...
        if (!canBuy) option.classList.add('disabled');
        option.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center"><span>🐾 ${pet.name}</span><span class="cost">${pet.price} 💰</span></div><div style="font-size:9px;color:#888">${bonusIcon} ${bonusText} | +10% EXP</div><div style="font-size:9px;color:#aaa;font-style:italic">${pet.desc}</div>`;
        if (canBuy) {
          const pId = petId;
          option.addEventListener('click', () => {
  ...
  buildQuestOptions(container) {
    ...
    for (const quest of activeQuests) {
      ...
      if (quest.completed && !quest.claimed) {
        option.innerHTML = `<span>🎁 ${quest.name}</span><span style="color:#4caf50">Nhận thưởng!</span>`;
        const qId = quest.id;
        option.addEventListener('click', () => {
        ...
        option.innerHTML = `<span>📜 ${quest.name}</span><span style="color:#888">${quest.current}/${quest.needed}</span>`;
        const qId = quest.id;
        option.addEventListener('click', () => {
  ...
  buyItem(itemId, price) {
    if (Player.gold < price) { UI.addLog('❌ Không đủ vàng!', 'system'); return false; }
    if (!Inventory.add(itemId, 1)) { UI.addLog('❌ Túi đồ đã đầy!', 'system'); return false; }
    Player.gold -= price;
    UI.updateGold();
  ...
  showTravelDialog(mapIndex) {
    ...
    confirmOption.innerHTML = `<span>✅ Dịch chuyển</span><span class="cost">${map.travelCost} 💰</span>`;
    if (canGo) {
...
// ===== CHANGES: Xóa 5 console.log debug trong click handlers (2 tab switch, 3 pet actions) =====
