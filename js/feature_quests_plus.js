...
const QuestsPlus = {
  ...
  _fillProcQuests() {
    // Keep unclaimed quests, remove old unclaimed beyond limit
    ...
    for (let i = 0; i < needed; i++) {
      ...
      let q;
      if      (type === 'chain')   q = this._generateChainQuest(Player.level);
      else if (type === 'collect') q = this._generateCollectQuest(Player.level, 'proc');
      else if (type === 'craft')   q = this._generateCraftQuest(Player.level);
      else                         q = this._generateKillQuest(Player.level, 'proc');
  ...
  _loadSave() {
    try {
      ...
      // Restore procQuests (chainTemplate set null — next chain step won't spawn, acceptable)
...
(function () {
  ...
  NPC.interact = function (npc) {
    if (npc.questPlusNpc) {
      ...
      Sprites.drawPixelArt(avatarCtx, Sprites.getNPCSprite('shop'), 2, 0, 0);
...
// ===== CHANGES: Xóa console.log debug trong init(); gom inline-object NPC push trong _spawnRealmNPCs thành 1 literal rõ hơn =====
