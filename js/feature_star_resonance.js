// ===== FILE: js/feature_star_resonance.js =====
...
const STAR_CHART_CONFIG = {
  ...
  pointsPerLevel:     (level) => level >= 10 ? 2 : 1,
  pointsPerRealm:     5,
...
const PET_RESONANCE_CONFIG = {
  ...
  resonanceSkills: {
    dog: {
      ...
      execute(target) {
        const dmg     = Math.floor(Player.atk * 3.5);
        const targets = Enemies.findInRange(target.x, target.y, 80);
        targets.forEach(item => {
          Enemies.damage(item.enemy,
            Math.floor(dmg * (item.enemy === target ? 1 : 0.5)),
    ...
    cat: {
      ...
      execute(target) {
        for (let i = 0; i < 5; i++) {
          setTimeout(() => {
            if (target.alive) {
              Enemies.damage(target, Math.floor(Player.atk * 1.2),
                Utils.chance(Player.critRate + 0.10), '#ffa500');
            }
          }, i * 120);
        }
      ...
      execute(target) {
        ...
        [-0.3, 0, 0.3].forEach(offset => {
          ...
          Enemies.list.forEach(e => {
            ...
            const ad   = Math.abs(((ea - angle) + Math.PI) % (2 * Math.PI) - Math.PI);
            if (ad < 0.2 && dist <= 200) {
              Enemies.damage(e, Math.floor(Player.atk * 2.0), Utils.chance(Player.critRate), '#87ceeb');
            }
          });
          for (let j = 0; j < 8; j++) {
...
var StarChartSystem = (function() {
  ...
  function getAllNodes() {
    ...
    for (var key in consts) {
      if (consts.hasOwnProperty(key)) consts[key].nodes.forEach(n => nodes.push(n));
    }
    STAR_CHART_CONFIG.centerNodes.forEach(n => nodes.push(n));
    state._allNodes = nodes;
    return nodes;
  }

  function canUnlock(nodeId) {
    var node = getAllNodes().find(n => n.id === nodeId);
    if (!node)                      return false;
    if (state.unlocked.has(nodeId)) return false;
    if (state.points < node.cost)   return false;
    return (node.requires || []).every(reqId => state.unlocked.has(reqId));
  }
  ...
  function reset(isFree) {
    ...
    if (isFree) {
      if (state.freeResetUsed) { UI.addLog('❌ Đã dùng Free Reset!', 'system'); return false; }
  ...
  function applyToStats(player) {
    ...
    Object.keys(player).filter(k => k.indexOf('_star') === 0).forEach(k => delete player[k]);

    state.unlocked.forEach(nodeId => {
      var node = getAllNodes().find(n => n.id === nodeId);
  ...
  function getSaveData() {
    return {
      points: state.points, totalEarned: state.totalEarned,
      unlocked: Array.from(state.unlocked), freeResetUsed: state.freeResetUsed
    };
  }
  ...
  return { state, getAllNodes, canUnlock, unlock, addPoints, reset, applyToStats, getSaveData, loadSaveData };
})();
...
var PetResonanceSystem = (function() {
  var state = {
    meter: 0, cooldownEndTime: 0, active: false,
    _playerAttackTimer: 0, _petAttackTimer: 0, _resonanceEndTime: 0
  };
  ...
  function checkSharedHit(target) {
    ...
    if (timeDiff <= 2000 && state._playerAttackTimer > 0 && state._petAttackTimer > 0 && target) {
      ...
      if (state.meter >= PET_RESONANCE_CONFIG.meterMax) triggerResonance();
    }
  }
  ...
  function triggerResonance() {
    ...
    var target = Player.target ||
      (Enemies ? Enemies.findNearest(Player.x, Player.y, 200, e => e.alive) : null);
    if (!target) return;
  ...
  function update(dt) {
    if (typeof AutoSystem !== 'undefined' && AutoSystem.active) {
      if (Player.activePet && Player.target?.alive) {
        state._playerAttackTimer = GameState.time;
        state._petAttackTimer    = GameState.time;
        checkSharedHit(Player.target);
      }
    }
    if (state.active && GameState.time > state._resonanceEndTime) state.active = false;

    const meterEl = document.getElementById('resonanceMeter');
    if (meterEl) meterEl.style.display = Player.activePet ? 'block' : 'none';
  }

  return { state, onPlayerAttack, onPetAttack, checkSharedHit, triggerResonance, update };
})();
...
var ResonanceMeterHUD = (function() {
  function update(meter) {
    ...
    if (trigBtn) trigBtn.style.display = (meter >= PET_RESONANCE_CONFIG.tapTriggerThreshold) ? 'block' : 'none';
  }
  return { update };
})();
...
var StarChartUI = (function() {
  ...
  function injectHTML() {
    ...
    overlay.style.cssText = 'display:none;position:fixed;inset:0;background:#080818;z-index:110;overflow:hidden;font-family:monospace';
    overlay.innerHTML = [
    ...
    document.getElementById('starClose').onclick     = close;
    document.getElementById('starFreeReset').onclick = () => StarChartSystem.reset(true);
    document.getElementById('starItemReset').onclick = () => StarChartSystem.reset(false);
  ...
  function _initBgStars() {
    ...
    for (var i = 0; i < 120; i++) {
      bgStars.push({ x: Math.random()*w, y: Math.random()*h, r: 0.5+Math.random()*1.5,
  ...
  function _animateBgStars() {
    if (document.getElementById('starChartOverlay').style.display === 'none') {
      requestAnimationFrame(_animateBgStars); return;
    }
    bgCtx.clearRect(0, 0, bgCanvas.width, bgCanvas.height);
    var t = Date.now() / 1000;
    bgStars.forEach(s => {
      bgCtx.globalAlpha = 0.3 + 0.7 * (0.5 + 0.5 * Math.sin(t * s.speed + s.phase));
  ...
  function findNodeAt(sx, sy) {
    ...
    for (var n of StarChartSystem.getAllNodes()) {
      var pos  = nodeScreenPos(n);
      var dist = Math.sqrt((sx - pos.x) ** 2 + (sy - pos.y) ** 2);
  ...
  function handleInteract(sx, sy) {
    ...
    if (!node || StarChartSystem.state.unlocked.has(node.id)) return;
    if (!StarChartSystem.canUnlock(node.id)) {
      var n = StarChartSystem.getAllNodes().find(x => x.id === node.id);
      UI.addLog(StarChartSystem.state.points < n.cost
        ? '❌ Không đủ Tinh Điểm! (cần ' + n.cost + ')' : '❌ Cần mở khóa các tinh tú trước!', 'system');
      return;
    }
  ...
  function onCanvasMouseMove(e) {
    ...
    if (node) {
      ...
      var missing = (node.requires || []).filter(r => !StarChartSystem.state.unlocked.has(r));
      ...
      document.getElementById('starTipReq').textContent  = missing.length ? '⚠ Cần: ' + missing.join(', ') : '';
  ...
  function render() {
    ...
    ['chien','thu','tri','van'].forEach(key => {
      var cung = consts[key];
      ctx.globalAlpha = 0.5; ctx.fillStyle = cung.color;
      ctx.font = 'bold 11px monospace'; ctx.textAlign = 'center';
      ctx.fillText(cung.icon + ' ' + cung.name, cung.position.cx * canvas.width, cung.position.cy * canvas.height - 40);
      ctx.globalAlpha = 1;
    });
    ...
    all.forEach(node => {
      ...
      (node.requires || []).forEach(reqId => {
        var req  = all.find(n => n.id === reqId);
        if (!req) return;
        var rpos = nodeScreenPos(req);
        ctx.beginPath(); ctx.moveTo(rpos.x, rpos.y); ctx.lineTo(pos.x, pos.y);
        ctx.strokeStyle = (unlocked.has(node.id) && unlocked.has(reqId)) ? 'rgba(240,192,64,0.6)' : 'rgba(255,255,255,0.12)';
        ctx.lineWidth   = (unlocked.has(node.id) && unlocked.has(reqId)) ? 1.5 : 1;
        ctx.stroke();
      });
    });

    // Center node edges (dashed)
    var centerX = canvas.width * 0.5, centerY = canvas.height * 0.5;
    STAR_CHART_CONFIG.centerNodes.forEach((cn, idx) => {
      ...
      (cn.requires || []).forEach(reqId => {
        var req = all.find(n => n.id === reqId);
        if (!req) return;
        var rpos = nodeScreenPos(req);
        ctx.beginPath(); ctx.moveTo(rpos.x, rpos.y); ctx.lineTo(nx, ny);
    ...
    all.forEach(node => {
      ...
      var color = '#888';
      ['chien','thu','tri','van'].forEach(key => {
        consts[key].nodes.forEach(n => { if (n.id === node.id) color = consts[key].color; });
      });

      ctx.shadowColor = (isUnlocked || isTwinkling) ? color : (canDo || isHovered) ? '#f0c040' : 'transparent';
      ctx.shadowBlur  = (isUnlocked || isTwinkling) ? (isTwinkling ? 20 : 8) : (canDo || isHovered) ? 6 : 0;
    ...
    STAR_CHART_CONFIG.centerNodes.forEach((cn, idx) => {
      ...
      var isHovered  = hoveredNode?.id === cn.id;
    ...
    if (unlocked.has('cm4')) {
      ctx.beginPath(); ctx.arc(centerX, centerY, 20 + Math.sin(Date.now() / 400) * 4, 0, Math.PI * 2);
      ctx.strokeStyle = 'rgba(240,192,64,0.6)'; ctx.lineWidth = 2; ctx.stroke();
    }
  }
  ...
  function animateUnlock(nodeId) {
    ...
    var ticks = 0;
    var iv = setInterval(() => { ticks++; render(); if (ticks > 20) clearInterval(iv); }, 60);
  }
  ...
  return { open, close, render, animateUnlock, updatePointsDisplay };
})();
...
var StarResonanceFeature = (function() {
  ...
  function injectResonanceMeter() {
    ...
    el.style.cssText = 'position:absolute;bottom:175px;right:25px;z-index:28;width:120px;pointer-events:auto;display:none';
    el.innerHTML = [
    ...
    document.getElementById('resonanceTrigger').onclick = () => PetResonanceSystem.triggerResonance();
  }

  function hookAvatarFrame() {
    var avCanvas = document.getElementById('avatarCanvas');
    if (avCanvas) { avCanvas.style.cursor = 'pointer'; avCanvas.addEventListener('click', e => { e.stopPropagation(); StarChartUI.open(); }); }
    var avFrame  = document.getElementById('avatarFrame');
    if (avFrame)  { avFrame.style.cursor  = 'pointer'; avFrame.addEventListener('click',  () => StarChartUI.open()); }
  }
  ...
  function patchGameSaveLoad() {
    ...
    Game.save = function() {
      _origSave();
      try { localStorage.setItem(STAR_CHART_CONFIG.storageKey, JSON.stringify(StarChartSystem.getSaveData())); }
  ...
  function patchGameUpdate() {
    ...
    Game.update = function(dt) {
      ...
      if (Player._starHpRegen) Player.hp = Math.min(Player.maxHp, Player.hp + Player._starHpRegen * dt / 1000);
      if (Player._starMpRegen) Player.mp = Math.min(Player.maxMp, Player.mp + Player._starMpRegen * dt / 1000);
    };
  }
  ...
  function patchEnemiesKill() {
    ...
    Enemies.kill = function(enemy) {
      if (Player._starGoldBonus) enemy.gold = Math.floor(enemy.gold * (1 + Player._starGoldBonus));
      if (Player._starExpBonus)  enemy.exp  = Math.floor(enemy.exp  * (1 + Player._starExpBonus));
      if (Player._starDropBonus && enemy.drops) {
        enemy.drops = enemy.drops.map(d => ({
          id: d.id, chance: Math.min(1, d.chance * (1 + Player._starDropBonus))
        }));
      }
      if (enemy.boss) StarChartSystem.addPoints(STAR_CHART_CONFIG.pointsPerBossEvent);
      _origKill(enemy);
      if (Player.activePet) PetResonanceSystem.onPetAttack(enemy);
    };
  }
  ...
  function patchPlayerGainExp() {
    ...
    Player.gainExp = function(amount) {
      ...
      if (Player.level > _prevLevel) {
        for (var i = 0; i < Player.level - _prevLevel; i++) {
          StarChartSystem.addPoints(STAR_CHART_CONFIG.pointsPerLevel(Player.level - (Player.level - _prevLevel) + i + 1));
        }
      }
    };
  }
  ...
  function patchPlayerUseSkill() {
    ...
    Player.useSkill = function(idx) {
      ...
      if (result && Player.target) PetResonanceSystem.onPlayerAttack(Player.target);
      return result;
    };
  }
  ...
  function patchInventoryUseItem() {
    ...
    Inventory.useItem = function(itemId) {
      ...
      if (itemData?.effect?.resetStarChart) return StarChartSystem.reset(false);
  ...
  function init() {
    ...
    // Inject HTML (open+close trick nếu chưa inject)
    if (!document.getElementById('starChartOverlay')) {
      StarChartUI.open();
      StarChartUI.close();
    }
    ...
    if (meterEl && Player.activePet) meterEl.style.display = 'block';

    setTimeout(hookAvatarFrame, 500);
  }

  var _origGameInit = Game.init.bind(Game);
  Game.init = function() { _origGameInit(); init(); };

  return { init };
})();
// ===== CHANGES: Xóa console.log; xóa IIFE wrapper thừa ở cuối (chỉ gọi lại _open() trong cả 2 nhánh — dead); rút gọn reset() — đảo isFree guard lên trước để loại bỏ else; rút gọn checkSharedHit — hợp nhất điều kiện if-lồng thành && chain; rút gọn update() — loại bỏ dereference thừa; rút gọn bird.execute dùng arrow function thay var+function; xóa _lastTargetId state (không được đọc); xóa comment load-order thừa =====
