...
const Player = {
  ...
  update(dt, inputX, inputY) {
    ...
    if (GameState.time % 2000 < dt) {
      this._preRegenHp = this.hp; // hook cho curse blockRegen
  ...
  updatePet(dt, isPlayerMoving) {
    ...
    switch (this.dir) {
      case 'left':  offsetX =  Math.abs(offsetX); break;
      case 'right': offsetX = -Math.abs(offsetX); break;
      case 'up':    offsetY =  Math.abs(offsetY); break;
      case 'down':  offsetY = -Math.abs(offsetY) + 20; break;
    }
  ...
  setTapTarget(worldX, worldY) {
    ...
    if (Maps.isWater(tileX, tileY)) return false;
  ...
  useSkill(idx) {
    ...
    switch (skill.type) {
      case 'melee':
      case 'projectile': targets = hitEnemies.slice(0, 1).map(h => h.enemy); break;
  ...
  die() {
    ...
    this.gold -= lostGold;
    if (lostGold > 0) UI.addLog(`💸 Mất ${lostGold} vàng!`, 'gold');
  ...
  // [FIX] Respawn tại trung tâm thành trấn
  respawn() {
    this.hp = this.maxHp;
    this.mp = this.maxMp;
    this.alive = true;

    // Spawn tại trung tâm vùng an toàn của map hiện tại
    // Guard: nếu Maps chưa ready thì dùng tọa độ mặc định
    if (Maps && Maps.tiles && Maps.tiles.length && typeof Maps.getTownBounds === 'function') {
      const town = Maps.getTownBounds();
      this.x = town.centerPx;
      this.y = town.centerPy;
    } else {
      this.x = 400;
      this.y = 400;
    }
    ...
    this.pet.x = this.x + this.pet.offsetX;
    this.pet.y = this.y + this.pet.offsetY;
    UI.addLog('✨ Đã hồi sinh trong vùng an toàn!', 'system');
  },
  ...
  setActivePet(petId) {
    ...
    this.pet.y = this.y + this.pet.offsetY;
    this.recalculateStats();
    UI.addLog(`🐾 Triệu hồi ${PETS[petId].name}!`, 'system');
    return true;
  },
  ...
  equip(itemId) {
    ...
    if (this.equipped[slot]) Inventory.add(this.equipped[slot], 1);
  ...
  heal(hp = 0, mp = 0) {
    if (hp > 0) {
      ...
      this.hp += healed;
      if (healed > 0) Game.spawnDamageNumber(this.x, this.y - 30, '+' + healed, '#44ff44');
    }
    if (mp > 0) this.mp = Math.min(this.maxMp, this.mp + mp);
  },
