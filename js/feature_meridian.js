...
const MeridianSystem = {
  ...
  _checkTongmach(id) {
    for (const adjId of (this.adjacent[id] || [])) {
      if (this.state[adjId].unlocked && !this.state[id].tongmach && !this.state[adjId].tongmach) {
        ...
        // Áp dụng thêm 1x nữa (x2 tổng cộng)
        this._applyMeridian(m,   false);
  ...
  // ── Bonus getters ──────────────────────────────────────────
  // Helper chung: tính tổng bonus theo predicate
  _sumBonus(predicate) {
    let bonus = 0;
    for (const m of this.meridians) {
      if (!this.state[m.id].unlocked) continue;
      if (predicate(m)) bonus += m.bonus * (this.state[m.id].tongmach ? 2 : 1);
    }
    return bonus;
  },

  getExpBonus() {
    return 1 + this._sumBonus(m => m.stat === 'expBonus' || m.stat === 'allBonus');
  },

  getAtkSpdBonus() {
    return 1 + this._sumBonus(m => m.stat === 'atkSpd');
  },

  getAllBonus() {
    return 1 + this._sumBonus(m => m.stat === 'allBonus');
  },
  ...
  _renderPanel() {
    ...
    for (const m of this.meridians) {
      ...
      if (unlocking) {
        ...
        arc.setAttribute('d', `M 0,-14 A 14,14 0 ${pct > 0.5 ? 1 : 0},1 ${x2},${y2}`);
    ...
    const unlockedCount  = this.meridians.filter(m => this.state[m.id].unlocked).length;
    const tongmachCount = this.meridians.filter(m => this.state[m.id].tongmach).length / 2;
    summary.innerHTML = `
      <div style="color:#4caf50;font-size:11px">✅ Đã khai: ${unlockedCount}/12</div>
  ...
  _showMeridianDetail(id) {
    ...
    const matsHtml = Object.entries(m.matCost).map(([matId, qty]) => {
    ...
    detail.innerHTML = `
      <div style="border-top:1px solid #333;margin-top:12px;padding-top:12px">
        ...
        ${s.unlocked
          ? `<div style="color:#4caf50;font-size:11px;margin-top:6px">✅ Đã khai mạch</div>`
          : unlocking
            ? `<div style="color:#2196f3;font-size:11px;margin-top:6px">⏳ Đang khai... còn ${remaining}s</div>`
...
(function () {
  ...
  // Wrap Player.gainExp để apply meridian expBonus
  ...
  // Wrap Player.useSkill để apply atkSpd meridian vào cooldown
...
// ===== CHANGES: =====
// 1. Hợp nhất 3 getter lặp code (getExpBonus, getAtkSpdBonus, getAllBonus) thành
//    dùng chung helper _sumBonus(predicate) — giảm ~30 dòng trùng lặp
// 2. Xóa biến trùng val = m.bonus * (tongmach?2:1) trong update() — inline trực tiếp
// 3. Xóa biến large thừa trong _renderPanel (arc progress) — inline biểu thức pct>0.5
// 4. Đơn giản hoá _showMeridianDetail: xóa biến effectVal không được dùng
// 5. Xóa comment sai ở đoạn wrap useSkill (comment gốc nói "Note: AtkSpd reduces cooldown
//    at end of useSkill / We wrap the cooldown application" nhưng thực ra wrap lại toàn bộ)
