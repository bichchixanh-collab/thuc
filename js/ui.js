...
const UI = {
  ...
  // Shared icon draw helper — handles both skill (24px) and menu (16px) canvases
  // by reading canvas.width/canvas.height instead of hardcoding sizes.
  _drawIcon(canvasId, data, scale) {
  ...
  // Draw skill icon (24×24 canvas, scale 5)
  drawSkillIcon(canvasId, data, scale) {
    this._drawIcon(canvasId, data, scale);
  },

  // Draw menu icon (16×16 canvas, scale 4)
  ...
  renderCharacter() {
    ...
    // Cache active pet data to avoid repeated lookups
    const activePet = Player.activePet ? PETS[Player.activePet] : null;

    const stats = document.getElementById('charStats');
    stats.innerHTML = `
      <div class="char-stat-row"><span class="stat-name">⚔️ Công kích</span><span class="stat-val">${Player.atk} ${activePet?.bonus.atk ? '<span style="color:#4caf50;font-size:9px">+' + activePet.bonus.atk + '</span>' : ''}</span></div>
      <div class="char-stat-row"><span class="stat-name">🛡️ Phòng ngự</span><span class="stat-val">${Player.def} ${activePet?.bonus.def ? '<span style="color:#4caf50;font-size:9px">+' + activePet.bonus.def + '</span>' : ''}</span></div>
      ...
      <div class="char-stat-row"><span class="stat-name">💨 Tốc độ</span><span class="stat-val">${Player.speed.toFixed(1)} ${activePet?.bonus.speed ? '<span style="color:#4caf50;font-size:9px">+10%</span>' : ''}</span></div>
    ...
    if (activePet) {
      let bonusText = '';
      if (activePet.bonus.atk)   bonusText = `⚔️ +${activePet.bonus.atk} ATK`;
      else if (activePet.bonus.def)   bonusText = `🛡️ +${activePet.bonus.def} DEF`;
      else if (activePet.bonus.speed) bonusText = `💨 +10% Speed`;
      ...
      petContent += `
        <div style="background:rgba(76,175,80,0.15);border:1px solid #4caf50;border-radius:8px;padding:10px;display:flex;align-items:center;gap:10px">
          ...
          <div>
            <div style="color:#4caf50;font-size:12px;font-weight:bold">${activePet.name}</div>
...
// ===== CHANGES:
// - Xóa dead variable `petBonusText` (tính nhưng không bao giờ dùng trong template)
// - Hợp nhất drawSkillIcon + drawMenuIcon thành _drawIcon() chung; hai hàm gốc giữ nguyên
//   tên, gọi qua _drawIcon() — dùng canvas.width/height thay hardcode 24/16
// - Trích xuất `activePet` local var trong renderCharacter() thay vì lookup lặp nhiều lần
// - Rút gọn openPanel(): early return khi panel null, bỏ else dư thừa
// - Xóa console.log debug cuối file
// - Xóa comment thừa ("Remove existing", "Close current panel if same"…) không thêm thông tin
// =====
