...
const RandomEventSystem = {
  ...
  update(dt) {
    ...
    if (s.activeEvent) {
      switch (s.activeEvent.type) {
        case 'treasure':  this._checkChestExpiry(dt); break;
  ...
  openChest() {
    ...
    if (Utils.dist(Player.x, Player.y, ev.x, ev.y) > WORLD_EVENT_CONFIG.chestOpenRange) return;
    ...
    UI.showNotification('📦 Mở Rương!', `Nhận được ${itemCount} vật phẩm & ${gold} vàng`);
  ...
  handleTap(wx, wy) {
    ...
    if (ev && ev.type === 'treasure' && !ev.isOpen) {
      if (Utils.dist(wx, wy, ev.x, ev.y) < WORLD_EVENT_CONFIG.chestOpenRange * 1.5) {
        this.openChest();
        return true;
      }
    }
