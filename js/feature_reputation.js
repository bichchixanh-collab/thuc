...
const ReputationSystem = {
  ...
  state: {
    reps: { kiemTong: 0, longToc: 0, thuongHoi: 0, daoMon: 0, maTong: 0 },
    mainFaction: null,
    activeBuffs: {},
    purchasedItems: {}
  },
  ...
  addRep(factionId, amount) {
    ...
    this.state.reps[factionId] = Math.min(5000, (this.state.reps[factionId] || 0) + amount);
  ...
  _updateBuffs() {
    for (const factionId in this.state.reps) {
      this.state.activeBuffs[factionId] = this.getTier(factionId).tier;
    }
    if (typeof Player !== 'undefined') Player.recalculateStats();
  },
    ...
    for (let t = 2; t <= tier.tier; t++) {
      ...
      shopItemIds.forEach(itemId => {
        ...
        opt.innerHTML =
          ...
          '<span>' + finalPrice + ' 💰' +
          (discount > 0 ? ' <small style="color:#4caf50">(-' + Math.round(discount * 100) + '%)</small>' : '') +
          '</span>';
  ...
  loadData() {
    try {
      ...
      if (data.reps)           Object.assign(this.state.reps, data.reps);
...
const TournamentSystem = {
  ...
  isTournamentTime(tournamentId) {
    ...
    const elapsed = Date.now() - (this.state.lastTournament[tournamentId] || 0);
  ...
  register(tournamentId) {
    ...
    if (typeof UI !== 'undefined') UI.addLog('📋 Đã đăng ký ' + sched.name + '!', 'realm');
    return true;
  },
  ...
  onMatchWin(tournamentId) {
    ...
    this.state.battleRecords.unshift({
      won: true, opp: this.state.matchData ? this.state.matchData.opponentName : '???', time: Date.now()
    });
  ...
  onTournamentWin(tournamentId) {
    ...
    if (typeof Player    !== 'undefined') Player.gold += reward.gold;
    if (typeof Inventory !== 'undefined') Inventory.add(reward.item, reward.count);
  ...
  onMatchLose(tournamentId) {
    ...
    this.state.battleRecords.unshift({
      won: false, opp: this.state.matchData ? this.state.matchData.opponentName : '???', time: Date.now()
    });
    if (this.state.battleRecords.length > 5) this.state.battleRecords.pop();
...
const TournamentArena = {
  ...
  update(matchData) {
    ...
    if (playerHpFill) playerHpFill.style.width = Math.max(0, matchData.player_hp / playerMaxHp * 100) + '%';
    if (playerHpText) playerHpText.textContent  = matchData.player_hp + '/' + playerMaxHp;

    const oppHpFill = document.getElementById('arenaOppHpFill');
    const oppHpText = document.getElementById('arenaOppHpText');
    if (oppHpFill) oppHpFill.style.width = Math.max(0, matchData.oppHp / matchData.oppMaxHp * 100) + '%';
    if (oppHpText) oppHpText.textContent  = matchData.oppHp + '/' + matchData.oppMaxHp;
  ...
  _startDmgLoop() {
    ...
    function loop() {
      ...
      ctx.beginPath(); ctx.arc(39, 10, 8, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = '#ef5350'; ctx.shadowColor = '#ef5350'; ctx.shadowBlur = 10;
      ctx.fillRect(152, 15, 18, 30);
      ctx.fillStyle = '#ff8a80';
      ctx.beginPath(); ctx.arc(161, 10, 8, 0, Math.PI * 2); ctx.fill();
      ctx.shadowBlur = 0;
      for (let i = self._dmgNumbers.length - 1; i >= 0; i--) {
        const dn = self._dmgNumbers[i];
        dn.life -= 16; dn.y += dn.vy;
        if (dn.life <= 0) { self._dmgNumbers.splice(i, 1); continue; }
        ctx.globalAlpha = Math.min(1, dn.life / 400);
  ...
  _stopDmgLoop() {
    if (this._dmgLoopId) { cancelAnimationFrame(this._dmgLoopId); this._dmgLoopId = null; }
  }
};
...
const ReputationPanel = {
  ...
  render() {
    ...
    for (const factionId in REPUTATION_CONFIG.factions) {
      ...
      const nextTier = REPUTATION_CONFIG.tiers[tier.tier];
      ...
      if (tier.tier >= 2) {
        btnRow.appendChild(_makeBtn('🛒 Cửa Hàng', f.color, () => {
          this._currentFactionShop = factionId; this.render();
        }));
      }

      if (!ReputationSystem.state.mainFaction && rep >= 100) {
        btnRow.appendChild(_makeBtn('🏛️ Gia Nhập', '#f0c040', () => {
          if (ReputationSystem.setMainFaction(factionId)) this.render();
        }));
      }
      ...
      if (q) {
        if (!q.active) {
          btnRow.appendChild(_makeBtn('📜 Nhiệm Vụ', '#88c', () => {
            ReputationSystem.acceptFactionQuest(factionId); this.render();
          }));
        } else {
          const isComplete = ReputationSystem.isFactionQuestComplete(factionId);
          if (isComplete) {
            btnRow.appendChild(_makeBtn('✅ Nhận Thưởng', '#4caf50', () => {
              ReputationSystem.claimFactionQuest(factionId); this.render();
            }));
          } else {
            ...
            questStatus.innerHTML = '📋 ' + _getFactionQuestProgress(factionId, q);
...
const TournamentPanel = {
  ...
  render() {
    ...
    TOURNAMENT_CONFIG.schedules.forEach(sched => {
      ...
      rewardInfo.innerHTML = '🎁 Phần thưởng vô địch: <span style="color:#f0c040">+' + reward.gold + ' 💰</span> | ' +
        (typeof ITEMS !== 'undefined' && ITEMS[reward.item] ? ITEMS[reward.item].name : reward.item) + ' x' + reward.count;
      ...
      if (isTime && !isReg) {
        btnRow.appendChild(_makeBtn('📋 Đăng Ký', '#f0c040', () => {
          TournamentSystem.register(sched.id); this.render();
        }));
      } else if (isReg) {
        btnRow.appendChild(_makeBtn('⚔️ Bắt Đầu', '#ef5350', () => {
          this.close();
    ...
    if (TournamentSystem.state.battleRecords.length > 0) {
      ...
      TournamentSystem.state.battleRecords.forEach(rec => {
        ...
        row.innerHTML = '<span>' + (rec.won ? '✅' : '❌') + ' vs ' + rec.opp + '</span><span style="color:#555">' + timeStr + '</span>';
...
<div id="repTournHudBtns" style="position:fixed;bottom:160px;right:8px;display:flex;flex-direction:column;gap:6px;z-index:50">
  ...
  _injectStyles() {
    ...
    style.textContent = `
      #reputationOverlay, #tournamentOverlay, #factionQuestOverlay { animation: repFadeIn 0.2s ease; }
      @keyframes repFadeIn { from { opacity:0; transform:scale(0.96); } to { opacity:1; transform:scale(1); } }
      #repHudBtn:hover, #tournHudBtn:hover { transform:scale(1.1); transition:transform 0.15s; }
  ...
  _buildArenaSkillButtons() {
    ...
    SKILLS.forEach((skill, idx) => {
      ...
      btn.addEventListener('click', () => _arenaUseSkill(idx));
    ...
    NPC.interact = function(npc) {
      ...
      if (npc.service === 'tournament_register') {
        ...
        if (dialogEl) {
          ...
          viewBtn.addEventListener('click', () => { NPC.closeDialog(); TournamentPanel.open(); });
  ...
  init() {
    ...
    if (typeof GhostPvPSystem !== 'undefined' && GhostPvPSystem.state && GhostPvPSystem.state.activeTitle) {
...
function _registerFactionNPCTypes() {
  const defs = {
    kiemTong:  { icon: '⚔️', title: 'Thanh Vân Kiếm Tông', sprite: 'npcTeleporter' },
    longToc:   { icon: '🐉', title: 'Long Tộc Liên Minh',  sprite: 'npcTeleporter' },
    thuongHoi: { icon: '💰', title: 'Vạn Bảo Thương Hội',  sprite: 'npcShop'       },
    daoMon:    { icon: '☯️', title: 'Huyền Thiên Đạo Môn', sprite: 'npcTeleporter' },
    maTong:    { icon: '👻', title: 'Ám Dạ Ma Tông',        sprite: 'npcTeleporter' }
  };
  for (const fid in defs) {
    const d = defs[fid];
    const f = REPUTATION_CONFIG.factions[fid];
    const q = ReputationSystem._factionQuests[fid];
    const questDesc = q && !q.active ? 'Ta có sứ mệnh cần ngươi thực hiện!' :
                      q && q.active  ? 'Nhiệm vụ đang tiến hành...' : '';
...
function _openFactionQuestPanel(factionId, npc) {
  ...
  if (q) {
    const questDescTexts = {
      kiemTong:  'Tiêu diệt ' + q.needed + ' quái vật để chứng tỏ sức mạnh kiếm tu!',
      longToc:   'Tiêu diệt ' + q.needed + ' kẻ địch nguy hiểm để bảo vệ lãnh thổ Long Tộc!',
      thuongHoi: 'Mua ' + q.needed + ' vật phẩm từ cửa hàng để thúc đẩy thương mại!',
      daoMon:    'Tu luyện tổng cộng ' + Math.floor(q.needed / 1000) + ' giây để tịnh hóa tâm hồn!',
      maTong:    'Tiêu diệt ' + q.needed + ' kẻ địch trong ban đêm để cống hiến hắc linh!'
    };
    ...
    questDesc.textContent = questDescTexts[factionId] || '';
    content.appendChild(questDesc);

    if (q.active) {
      const progEl = document.createElement('div');
      progEl.style.cssText = 'font-size:9px;color:#888;margin-bottom:8px';
      progEl.textContent = '📋 Tiến Độ: ' + _getFactionQuestProgress(factionId, q);
      content.appendChild(progEl);
    }
...
function _arenaUseSkill(idx) {
  ...
  if (match.oppHp <= 0) TournamentSystem.onMatchWin(match.tournamentId);
}
...
// ===== CHANGES: Không thay đổi logic — source khớp với batch7; giữ nguyên toàn bộ =====
